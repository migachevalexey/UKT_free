...
def sql_db_select():  # надо переделать на Pandas pd.read_sql
    ...
    sqlQueryItem = f"SELECT i.item_id, i.original_id, replace(replace(i.name,',' , '.'), '\n',''),replace(c.name,',','.') as category, replace(pv.value,',','.') as brand " \
                "from ITEM i " \
                "left join CATALOg c on c.id = i.catalogue_id " \
                "left join ukt_prop_mark pm on pm.item_id = i.item_id and pm.property_id = 479 " \
                "left join ukt_prop_val pv on pv.id = pm.property_value_id " \
...
def queryData(QUERY, tab):
    ...
    if QUERY.upper().find('ERGE') != -1:
        print(query_job.state, query_job.num_dml_affected_rows, "rows affected")
        with open(r'c:\python\ukt\Cron\log\Item_Action_toBQ.txt', 'a') as f:
            f.write(f'На дату: {datetime.date.today()} ' + ' загружено ' + str(query_job.num_dml_affected_rows) + f' строк в таблицу {tab} '+ query_job.state+'\n')
    return rows
