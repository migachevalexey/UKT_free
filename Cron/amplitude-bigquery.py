...
'''На еженднефвной основе вытягивает данные из сервися Amplitude и стрими все это в BigQuery'''
...
def sendMail(l):
    ...
    with open(r'C:\Python\ukt\pass\mail.txt') as f:
        p = f.readline()
...
# Не работает так!! Perform a CURL request to download the export from Amplitude
# os.system("curl -u "+API_KEY+":"+API_SECRET+" https://amplitude.com/api/2/export?start="+YESTERDAY+"T01&end="+YESTERDAY+"T23 >> amplitude.zip")
...
def main():
    ...
    with open('amplitude.zip', 'wb') as f:
        f.write(r.content)
    # удаляем если осталось что то старое
    mydir = r'C:\Python\ukt\Cron\amplitude\import'
    filelist = [f for f in os.listdir(mydir)]
    for f in filelist:
        os.remove(os.path.join(mydir, f))
    mydir = fr'C:\Python\ukt\Cron\amplitude\{ACCOUNT_ID}'
    filelist = [f for f in os.listdir(mydir)]
    for f in filelist:
        os.remove(os.path.join(mydir, f))
